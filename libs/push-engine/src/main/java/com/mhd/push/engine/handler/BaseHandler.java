...
import com.mhd.push.engine.flowcontrol.FlowControlFactory;
import com.mhd.push.engine.flowcontrol.FlowControlParam;
...
public abstract class BaseHandler implements Handler {
    ...
    /**
     * 使用统一模板记录渠道发送失败日志，并同步写入 trace/timeLine。
     *
     * @param taskInfo 任务信息
     * @param detail 失败详情
     */
    protected void logHandlerFailure(TaskInfo taskInfo, Object detail) {
        // 控制台日志统一带齐关键排查字段，便于按 traceId、渠道、账号快速检索。
        String detailText = detail == null ? "" : JSON.toJSONString(detail);
        logUtils.printTraceNode(taskInfo, "CHANNEL_SEND", "FAIL", "渠道发送失败: " + detailText, "ERROR");
        log.error("handler stage=channel_send result=fail traceId={} channel={} accountId={} receiverCount={} handler={} detail={}",
                taskInfo == null ? null : taskInfo.getTraceId(),
                taskInfo == null ? channelCode : taskInfo.getSendChannel(),
                taskInfo == null ? null : taskInfo.getSendAccount(),
                taskInfo == null || taskInfo.getReceiver() == null ? 0 : taskInfo.getReceiver().size(),
                getClass().getSimpleName(),
                detailText);
    }

    /**
     * 使用统一模板记录渠道发送异常日志，并同步写入 trace/timeLine。
     *
     * @param taskInfo 任务信息
     * @param detail 失败详情
     * @param throwable 异常对象
     */
    protected void logHandlerFailure(TaskInfo taskInfo, String detail, Throwable throwable) {
        // trace 中保留异常摘要，admin 页面可以看到具体失败节点与原因。
        logUtils.printTraceNode(taskInfo, "CHANNEL_SEND", "FAIL", "渠道发送异常: " + detail, "ERROR");
        log.error("handler stage=channel_send result=fail traceId={} channel={} accountId={} receiverCount={} handler={} detail={}",
                taskInfo == null ? null : taskInfo.getTraceId(),
                taskInfo == null ? channelCode : taskInfo.getSendChannel(),
                taskInfo == null ? null : taskInfo.getSendAccount(),
