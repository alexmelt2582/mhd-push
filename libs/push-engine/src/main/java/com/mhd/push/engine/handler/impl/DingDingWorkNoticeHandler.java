...
import com.mhd.push.domain.model.account.DingDingWorkNoticeAccount;
import com.mhd.push.domain.model.task.RecallTaskInfo;
import com.mhd.push.domain.model.task.TaskInfo;
import com.mhd.push.common.dto.model.DingDingWorkContentModel;
...
import com.mhd.push.infra.config.SupportThreadPoolConfig;
import com.mhd.push.infra.thirdparty.AccessTokenUtils;
...
public class DingDingWorkNoticeHandler extends BaseHandler {
    ...
    public boolean handler(TaskInfo taskInfo) {
        try {
            ...
            if (response.isSuccess()) {
                saveRecallInfo(DING_DING_RECALL_KEY_PREFIX, taskInfo.getTemplateId(), String.valueOf(response.getTaskId()), CommonConstant.ONE_DAY_SECOND);
                return true;
            }
    ...
    private OapiMessageCorpconversationAsyncsendV2Request assembleParam(DingDingWorkNoticeAccount account, TaskInfo taskInfo) {
        ...
        try {
            // 接收者相关
            if (GlobalConstant.SEND_ALL.equals(CollUtil.getFirst(taskInfo.getReceiver()))) {
